...
with row1[0]:
    st.image('web2.png', use_column_width="auto", width=300)
   ...
   # Predict button
    if st.button("Predict"):
        ...
        if (pulse == 0 or respiration == 0.0 or bloodureanitro == 0.0 or neutrophils == 0.0 or 
            ...
            # Recalculate RMSE and R2 with the new patient data
            y_pred_combined = model.predict(X_combined)
            rmse_all = np.sqrt(mean_squared_error(y_combined, y_pred_combined))
            r2_all = r2_score(y_combined, y_pred_combined)
    ...
    if rounded_prediction is not None and discharge_date is not None:
       st.markdown(
        ...
        <div class="output-box">
            ...
            <p>R² Score for all data: {r2_all:.6f}</p>
        </div>
        """,
        unsafe_allow_html=True
    )
